...
public partial class AccountingDbContext
{
    ...
    public DbSet<DbExpenseDetail> ExpenseDetails => Set<DbExpenseDetail>();

    public DbSet<DbExpenseDetailItem> ExpenseDetailItems => Set<DbExpenseDetailItem>();
    ...
    private static void BuildExpensesModel(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<DbExpenseDetail>()
            .HasMany(x => x.Items)
            .WithOne(x => x.Detail)
        ...
        modelBuilder.Entity<DbExpenseDetail>().HasQueryFilter(x => !x.Deleted);
        modelBuilder.Entity<DbExpenseDetailItem>().HasQueryFilter(x => !x.Deleted);
        ...
        foreach (string money in new[]
                 {
                     nameof(DbExpenseDetail.TotalIncGst),
                     nameof(DbExpenseDetail.GstAmount),
                     nameof(DbExpenseDetail.NonReimbursedAmount)
                 })
        {
            modelBuilder.Entity<DbExpenseDetail>().Property(money).HasPrecision(12, 2);
        }

        modelBuilder.Entity<DbExpenseDetailItem>().Property(x => x.Amount).HasPrecision(12, 2);

        // There is no ChurchUsePercent any more. Church use is a per-item yes/no, because a percentage
        // means something about a whole basket and nothing about a packet of paper plates.
        ...
        // Details are always fetched for one submission, in the order they were attached.
        modelBuilder.Entity<DbExpenseDetail>().HasIndex(x => new { x.SubmissionId, x.Ordinal });

        // How an attachment finds its detail, and vice versa. NOT unique: a rewritten draft leaves the
        // superseded (soft-deleted) rows behind carrying the same key, which is exactly what the key
        // being stable means.
        modelBuilder.Entity<DbExpenseDetail>().HasIndex(x => new { x.SubmissionId, x.Key });
        modelBuilder.Entity<DbExpenseAttachment>().HasIndex(x => new { x.SubmissionId, x.DetailKey });

        modelBuilder.Entity<DbExpenseDetailItem>().HasIndex(x => new { x.DetailId, x.Ordinal });
