...
```yaml
---
title: The expense submission model      # sentence case, matches the H1
kind: reference                          # reference | plan | handover | discussion | investigation
status: current                          # see the lifecycle below
module: expenses                         # omit only for docs/README.md
opened: 2026-08-31                       # work/ and open-questions/ only
verified: 2026-08-31                     # last date someone checked this against the code
...
5. **Run `./update-sln-docs.sh`** from the repo root. It regenerates the docs solution folders in
   `GSBC.Accounting.sln` from the filesystem, so the file shows up in Rider without "Show all files".
...
- This is a church finance app: when a doc describes something a claimant or an approver sees, say what
  they see. A rule like "the approver must not be the claimant" is a compliance constraint, not a
  preference.
